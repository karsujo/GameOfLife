...
function computeNeighborValues(context, x, y) {
    ...
    if (xpos > 0 && ypos > 0 && xpos < canvas.width && ypos < canvas.height) {
        ...
        entities[0] = createEntity(xpos, ypos, IsEntityWhite(context, xpos, ypos) == true ? 'alive' : 'dead');
    }
    (xpos = x), (ypos = y + boxUnit);
    if (xpos >= 0 && ypos > 0 && xpos < canvas.width && ypos < canvas.height) {
        entities[1] = createEntity(xpos, ypos, IsEntityWhite(context, xpos, ypos) == true ? 'alive' : 'dead');
    }
    (xpos = x + boxUnit), (ypos = y + boxUnit);
    if (xpos >= 0 && ypos > 0 && xpos < canvas.width && ypos < canvas.height) {
        entities[2] = createEntity(xpos, ypos, IsEntityWhite(context, xpos, ypos) == true ? 'alive' : 'dead');
    }
    (xpos = x + boxUnit), (ypos = y);
    if (xpos >= 0 && ypos > 0 && xpos < canvas.width && ypos < canvas.height) {
        entities[3] = createEntity(xpos, ypos, IsEntityWhite(context, xpos, ypos) == true ? 'alive' : 'dead');
    }
    (xpos = x - boxUnit), (ypos = y + boxUnit);
    if (xpos >= 0 && ypos > 0 && xpos < canvas.width && ypos < canvas.height) {
        entities[4] = createEntity(xpos, ypos, IsEntityWhite(context, xpos, ypos) == true ? 'alive' : 'dead');
    }
    (xpos = x), (ypos = y - boxUnit);
    if (xpos >= 0 && ypos > 0 && xpos < canvas.width && ypos < canvas.height) {
        entities[5] = createEntity(xpos, ypos, IsEntityWhite(context, xpos, ypos) == true ? 'alive' : 'dead');
    }
    (xpos = x - boxUnit), (ypos = y - boxUnit);
    if (xpos >= 0 && ypos > 0 && xpos < canvas.width && ypos < canvas.height) {
        entities[6] = createEntity(xpos, ypos, IsEntityWhite(context, xpos, ypos) == true ? 'alive' : 'dead');
    }
    (xpos = x - boxUnit), (ypos = y);
    if (xpos >= 0 && ypos > 0 && xpos < canvas.width && ypos < canvas.height) {
        entities[7] = createEntity(xpos, ypos, IsEntityWhite(context, xpos, ypos) == true ? 'alive' : 'dead');
    }
    //current cell
    (xpos = x), (ypos = y);
    if (xpos >= 0 && ypos > 0 && xpos < canvas.width && ypos < canvas.height) {
        entities[8] = createEntity(xpos, ypos, IsEntityWhite(context, xpos, ypos) == true ? 'alive' : 'dead');
    }
    return entities;
}
